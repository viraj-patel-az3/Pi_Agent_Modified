	...
	it("rejects an existing fork target session id", async () => {
		const result = await runCli(
			...
			(dirs) => {
				...
				const realProjectDir = realpathSync(dirs.projectDir);
				writeSession(dirs.sessionDir, realProjectDir, "source-id");
				writeSession(dirs.sessionDir, realProjectDir, "existing-id");
			},
		);
