...
### fix: {=...} inline expression — show result then pass to LLM
- Removed early return from inline `{=...}` substitution handler
- Interpolated text is now previewed to the user AND forwarded to the LLM pipeline
- Bare `=` interceptor is unchanged — still short-circuits before LLM
- Only `agent-session.ts` modified, no other files touched

### fix: {=...} preview routes through sendCustomMessage without label
- Replaced writeRawStdout with sendCustomMessage so preview appears in TUI output
- Removed [preview] prefix — substituted text displays cleanly with no label
- display: true ensures it renders in all modes (interactive, print, rpc)
