...
import type { AgentSession, CreateAgentSessionOptions } from "./core/sdk.ts";
import {
...
async function prepareInitialMessage(
	...
	// Viraj's code — batch prompt file support
	// Splits @packages/coding-agent/examples/extensions/file-trigger.ts content by \n---\n separator into multiple prompts
	// Single prompt files behave identically to before (no regression)
	// Multiple prompts are queued using existing _queueFollowUp() mechanism
	...
	for (const fileArg of parsed.fileArgs) {
		try {
			...
			// Ignore read errors (e.g. for image files)
		}
	}
	...
	if (allPrompts.length > 1) {
		...
		if (session) {
			for (const followUp of allPrompts.slice(1)) {
				(session as any)._queueFollowUp(followUp);
			}
		}
		return buildInitialMessage({
			parsed,
			fileText: firstPrompt,
			stdinContent,
		});
	}
...
export async function main(args: string[], options?: MainOptions) {
	...
	// if (process.env.PI_TEST_ERROR === "1") {
	// 	// for testing
	// 	emit({ type: "error", message: "This is a testing error" });
	// }
	...
	if (appMode === "rpc") {
		...
		const interactiveMode = new InteractiveMode(runtime, {
			...
			// Viraj's Code start
			startupHelpMode: parsed.startupHelpMode,
			// Viraj's Code end
		});
		if (startupBenchmark) {
